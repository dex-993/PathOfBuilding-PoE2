...
function buildMode:Load(xml, fileName)
	...
	for _, child in ipairs(xml) do
		...
		elseif child.elem == "TreeConstraints" then
			self.treeConstraints = {
				includeNodeNames = { },
				includeJewelCategories = { },
			}
			if child.attrib.includeNodeNames then
				for name in child.attrib.includeNodeNames:gmatch("[^,]+") do
					self.treeConstraints.includeNodeNames[name] = true
				end
			end
			if child.attrib.includeJewelCategories then
				for cat in child.attrib.includeJewelCategories:gmatch("[^,]+") do
					self.treeConstraints.includeJewelCategories[cat] = true
				end
			end
		end
	end
...
function buildMode:Save(xml)
	...
	local treeConstraints = self.treeConstraints
	if treeConstraints then
		local includeNodeNames = ""
		for name in pairs(treeConstraints.includeNodeNames or { }) do
			includeNodeNames = includeNodeNames .. (includeNodeNames ~= "" and "," or "") .. name
		end
		local includeJewelCategories = ""
		for cat in pairs(treeConstraints.includeJewelCategories or { }) do
			includeJewelCategories = includeJewelCategories .. (includeJewelCategories ~= "" and "," or "") .. cat
		end
		if includeNodeNames ~= "" or includeJewelCategories ~= "" then
			t_insert(xml, {
				elem = "TreeConstraints",
				attrib = {
					includeNodeNames = includeNodeNames,
					includeJewelCategories = includeJewelCategories,
				}
			})
		end
	end
