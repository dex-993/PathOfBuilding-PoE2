...
function TreeTabClass:AutoAllocateTreeConfirmed(candidateCount)
	...
	self.build.autoAllocateBuilder = coroutine.create(function()
		...
		coroutine.yield()		
		-- ========================================================================
		-- PHASE 3d: Optimize attribute node selection (Str/Dex/Int)
		-- Try each allocated attribute node with all 3 options and pick the best.
		-- ========================================================================
		self.build.autoAllocateProgress = "Phase 3d: Optimizing attribute selections..."
		coroutine.yield()
		local attrChanges = 0
		local attrIdx = 0
		for _, entry in ipairs(allocated) do
			local node = entry.node
			if node.alloc and node.isAttribute and node.options and #node.options >= 3 then
				-- Determine current attribute index by checking the live node
				local currentDn = spec.nodes[node.id].dn
				local currentIdx = 1
				for oi, opt in ipairs(node.options) do
					if opt.name == currentDn then
						currentIdx = oi
						break
					end
				end

				-- Save hashOverride reference for restore if default is best
				local savedOverride = spec.hashOverrides and spec.hashOverrides[node.id]

				-- Try each option and pick the one with highest damage
				local bestDmg = -math.huge
				local bestIdx = currentIdx
				for oi = 1, #node.options do
					spec:SwitchAttributeNode(node.id, oi)
					spec:BuildAllDependsAndPaths()
					local out = calcFunc({ }, false)
					local dmg = out.AverageDamage or 0
					if dmg > bestDmg then
						bestDmg = dmg
						bestIdx = oi
					end
				...
				-- Apply best option (we are on the last option tried)
				if bestIdx ~= currentIdx then
					spec:SwitchAttributeNode(node.id, bestIdx)
					spec:BuildAllDependsAndPaths()
					attrChanges = attrChanges + 1
				else
					-- Restore original state
					spec.hashOverrides[node.id] = savedOverride
					spec:BuildAllDependsAndPaths()
				end
			end
			attrIdx = attrIdx + 1
			if attrIdx % 5 == 0 then
				self.build.autoAllocateProgress = string.format(
					"Phase 3d: Optimizing attributes... %d / %d", attrIdx, #allocated)
				coroutine.yield()
			end
		end

		if attrChanges > 0 then
			spec:AddUndoState()
			self.build.buildFlag = true
		end
		self.build.autoAllocateProgress = string.format(
			"Phase 3d: Changed %d attribute nodes", attrChanges)
		coroutine.yield()
